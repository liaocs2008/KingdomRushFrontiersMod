...
bool Twister::init()
{
	...
	addChild(sprite);
	nearestMonster = NULL;
	scheduleOnce(CC_SCHEDULE_SELECTOR(Twister::removeTwisterAction), 5.0f);
	return true;
}
void Twister::round()
{
...
void Twister::attackMon()
{
	...
	schedule(CC_SCHEDULE_SELECTOR(Twister::hitMon), 0.5f);
	schedule(CC_SCHEDULE_SELECTOR(Twister::move), 1.0f);
}
