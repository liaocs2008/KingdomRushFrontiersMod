...
void Level4::addOrnament()
{
	...
	for(int i=0;i<4;i++)
	{
		treeTerrains[i]=Sprite::createWithSpriteFrameName(StringUtils::format("Stage4_Trees_Terrains_000%d.png",i+1));
...
void Level4::waveEvent()
{
	if(wave == 0){
		...
		for (int i = 1; i <= 9; i++)
		{
			frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("Stage4_ShipDoor_00%02d.png",i));
			if(frame!=nullptr)
