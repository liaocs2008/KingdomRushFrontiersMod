...
bool PlayerStateMenu::init()
{
	...
	for(int i = 0; i < 6 ; i ++){
		int num = UserDefault::getInstance()->getIntegerForKey(StringUtils::format(instance->SLOTX_SHOP,i).c_str());
		if(num > 0){
			stringForSkillFileName = StringUtils::format("backPack_icons_000%d.png",i+1);
		}else{
		...
		backPack_icons_Sprite[i]->setName(StringUtils::format(instance->SLOTX_SHOP,i));
		backPackSprite->addChild(backPack_icons_Sprite[i],0);

		auto numLeft = Label::createWithTTF(StringUtils::format("%d",num),"arial.ttf",20);
	...
	skillListener->onTouchBegan = [&](Touch* touch, Event* event){
		...
		if (rect.containsPoint(locationInNode) && backPackSprite->isVisible()){  
			...
			if(num <= 0){
				...
				target->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format( "backPack_icons_off_00%02d.png",target->getTag()+1)));
				...
				num--;
				static_cast<Label*>(target->getChildByTag(101))->setString(StringUtils::format("%d",num));
...
{
	lifeLabel->setString(StringUtils::format("%d",life));
}
...
{
	waveLabel->setString(StringUtils::format("WAVE %d/%d",thisWave,totalWave));
}
...
void PlayerStateMenu::updateStoneProgress(float Dt){  
	...
	// add by leo
	stoneTimer->setPercentage(0);

	//if (stoneTimer->getPercentage()==0) {
	if (stoneTimer->getPercentage() <= 0) {
		this->unschedule(CC_SCHEDULE_SELECTOR(PlayerStateMenu::updateStoneProgress));//取消定时器
...
void PlayerStateMenu::updateParatrooperProgress(float Dt){ 
	...
	if (paratrooperTimer->getPercentage()==0) {
		this->unschedule(CC_SCHEDULE_SELECTOR(PlayerStateMenu::updateParatrooperProgress));//取消定时器
