...
void GameViewMenu::onEnterTransitionDidFinish()
{
	...
	starLabel->setString(StringUtils::format("%d/65", UserDefault::getInstance()->getIntegerForKey(instance->SLOTX_STAR,0)));
	diamondsLabel->setString(StringUtils::format("%d", UserDefault::getInstance()->getIntegerForKey(instance->SLOTX_GEM,0)));
	if(isFirst){
