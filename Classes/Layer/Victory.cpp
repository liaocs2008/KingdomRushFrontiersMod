...
bool Victory::initVictory(int star,int gem)
{
	...
	auto one = Sprite::createWithSpriteFrameName(StringUtils::format("win_Gnome_Numbers_000%d.png",gem%10));
	auto ten = Sprite::createWithSpriteFrameName(StringUtils::format("win_Gnome_Numbers_000%d.png",(gem/10)%10));
	...
	{
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("victoryStars_star_00%02d.png",len));
		if(frame!=nullptr)
