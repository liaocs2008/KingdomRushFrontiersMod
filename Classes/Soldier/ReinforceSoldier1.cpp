...
void ReinforceSoldier1::birth(Point point)
{
	setPosition(point);
	scheduleUpdate();
	schedule(CC_SCHEDULE_SELECTOR(ReinforceSoldier1::lookingForMonsters), 1.0f,-1,caculateTime(point));
}
...
void ReinforceSoldier1::attackMonster(float dt)
{
	if(nearestMonster!=NULL && nearestMonster->getCurrHp()>0){
		...
		if(monsterCurrHp == 0){
			unschedule(CC_SCHEDULE_SELECTOR(ReinforceSoldier1::attackMonster));
...
void ReinforceSoldier1::runToMonster()
{
	...
	schedule(CC_SCHEDULE_SELECTOR(ReinforceSoldier1::attackMonster), 1.0f,-1,caculateTime(destination));
}
