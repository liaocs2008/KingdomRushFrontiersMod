...
void ReinforceSoldier2::birth(Point point)
{
	setPosition(point);
	scheduleUpdate();
	schedule(CC_SCHEDULE_SELECTOR(ReinforceSoldier2::lookingForMonsters), 1.0f,-1,caculateTime(point));
}
...
void ReinforceSoldier2::attackMonster(float dt)
{
	if(nearestMonster!=NULL && nearestMonster->getCurrHp()>0){
		...
		if(monsterCurrHp == 0){
			unschedule(CC_SCHEDULE_SELECTOR(ReinforceSoldier2::attackMonster));
...
void ReinforceSoldier2::runToMonster()
{
	...
	schedule(CC_SCHEDULE_SELECTOR(ReinforceSoldier2::attackMonster), 1.0f,-1,caculateTime(destination));
}
