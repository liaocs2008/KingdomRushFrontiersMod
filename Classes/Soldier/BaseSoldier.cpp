...
void BaseSoldier::runToLocation(Point point)
{
	if(getState()!=stateDeath){
		...
		if (hslSoldierType != HSLSoldierType::HSL_NA) {
			baseSprite->setFlippedX(false);
			if ((point.x - this->getPositionX())>0) { //ÓÒ±ß
				setState(SoldierStateRunRight);
			}
			else {
				setState(SoldierStateRunLeft);
			}
		}
		else {
...
bool BaseSoldier::init()
{
	...
	nearestMonster = NULL;
	hslSoldierType = HSLSoldierType::HSL_NA;
	return true;
}
...
void BaseSoldier::lookingForMonsters(float dt)
{
	...
	// add by leo, we don't need this feature
	if (hslSoldierType != HSLSoldierType::HSL_NA) {
		baseSprite->setFlippedX(false);
	}
	else {
