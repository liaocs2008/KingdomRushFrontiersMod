...
void Assassin::cheakState()
{
	if(nearestMonster!=NULL){
		scheduleUpdate();
		schedule(CC_SCHEDULE_SELECTOR(Assassin::attackMonster), 1.0f,-1,0);
	}else{
...
void Assassin::attackMonster(float dt)
{
	if(nearestMonster!=NULL && nearestMonster->getCurrHp()>0){
		...
		if(monsterCurrHp == 0){
			unschedule(CC_SCHEDULE_SELECTOR(Assassin::attackMonster));
