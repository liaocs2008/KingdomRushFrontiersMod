...
void Skeleton::birth(Point point)
{
	...
	baseSprite->runAction(Sequence::create(
		...
		,CallFuncN::create(CC_CALLBACK_0(Skeleton::setState, this,SoldierStateWait))
                , NULL));
	schedule(CC_SCHEDULE_SELECTOR(Skeleton::lookingForMonsters), 1.0f,-1,caculateTime(point));
}
...
void Skeleton::attackMonster(float dt)
{
	if(nearestMonster!=NULL && nearestMonster->getCurrHp()>0){
		...
		if(monsterCurrHp == 0){
			unschedule(CC_SCHEDULE_SELECTOR(Skeleton::attackMonster));
...
void Skeleton::runToMonster()
{
	...
	schedule(CC_SCHEDULE_SELECTOR(Skeleton::attackMonster), 1.0f,-1,caculateTime(destination));
}
