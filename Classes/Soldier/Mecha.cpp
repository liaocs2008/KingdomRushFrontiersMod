...
bool Mecha::init()
{
	...
	addChild(baseSprite);
	addChild(shit);
	schedule(CC_SCHEDULE_SELECTOR(Mecha::bombMonster), 1.5f);
	return true;
}
...
void Mecha::addShit()
{
	...
	shit->shoot();
	couldShit = false;
	scheduleOnce(CC_SCHEDULE_SELECTOR(Mecha::shitCD), 10.0f);
}
...
void Mecha::runToLocation(Point point)
{
	...
	unschedule(CC_SCHEDULE_SELECTOR(Mecha::bombMonster));
	...
	schedule(CC_SCHEDULE_SELECTOR(Mecha::bombMonster), 1.5f,-1,caculateTime(point));
}
