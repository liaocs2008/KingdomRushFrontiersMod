...
// add by leo
#include "Soldier/BarrackHSLSoldier.h"
...
{
	towerBase = Sprite::createWithSpriteFrameName(StringUtils::format("tower_barracks_lvl%d_layer1_0001.png",level));
	door = Sprite::createWithSpriteFrameName(StringUtils::format("tower_barracks_lvl%d_layer2_0001.png",level));
	...
	*/
	...
	auto Soldier_1 = BarrackHSLSoldier::createSoldier(Point(60, -30), level, HSLSoldierType::HSL_TENA);
	auto Soldier_2 = BarrackHSLSoldier::createSoldier(Point(-60, -30), level, HSLSoldierType::HSL_HANDS);
	auto Soldier_3 = BarrackHSLSoldier::createSoldier(Point(0, -60), level, HSLSoldierType::HSL_FRANK);
	...
	door->runAction(Animate::create(AnimationCache::getInstance()->getAnimation(StringUtils::format("level%d_barracks_opendoor",getLevel()))));

	schedule(CC_SCHEDULE_SELECTOR(BaseBarracksTower::cureSoldiers), 10.0f,-1,10.0f);
}
...
void BaseBarracksTower::cureSoldiers(float dt)
{
	for(int i = 0;i<Soldiers.size();i++)
	{
		...
		if(Soldier->getState() == SoldierStateWait){
			...
			// add by leo
			auto s = Soldier->getHSLSoldierType();
			...
			auto newSoldier = BarrackHSLSoldier::createSoldier(point, level, s);
...
void BaseBarracksTower::buildingAnimation()
{
	...
	hpBar->runAction(Sequence::create(ProgressTo::create(0.5f, 100)
		, CallFuncN::create(CC_CALLBACK_0(Sprite::removeFromParent, building))
                                           , NULL));
	scheduleOnce(CC_SCHEDULE_SELECTOR(BaseBarracksTower::buildingSmokeAnimation),0.5f);
}
...
void BaseBarracksTower::showUpdateMenu()
{
	if(level < 3){
		...
		setUpdateMoney(finalStateUpdateMoney);
		auto updatePanleLayer = UpdatePanleLayer2WithFlag::createPanle(24, 23, finalStateUpdateMoney, finalStateUpdateMoney);
...
void BaseBarracksTower::update1()
{
	...
	buildMoney += finalStateUpdateMoney;
...
void BaseBarracksTower::updateTower()
{
	...
	towerBase -> setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("tower_barracks_lvl%d_layer1_0001.png",level)));
	door->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("tower_barracks_lvl%d_layer2_0001.png",level)));
	for(int i = 0; i< Soldiers.size() ;i++)
	{
		Soldiers.at(i)->updateSoldier(level);
	}
...
void BaseBarracksTower::setRallyPoint(Point point)
{
	...
	{
		if (i == 0)
			Soldiers.at(0)->setLocation(Point(rally.x + 30, rally.y - 30));
		else if (i == 1)
			Soldiers.at(1)->setLocation(Point(rally.x - 30, rally.y - 30));
		else if (i == 2)
			Soldiers.at(2)->setLocation(Point(rally.x, rally.y + 30));
		/*
		if (i==0)
