...
void BaseMageTower::initTower(int level)
{
	if(level<4)
	{
		towerBase=Sprite::createWithSpriteFrameName(StringUtils::format("mage_lvl%d_0001.png",level));
		shooter=Sprite::createWithSpriteFrameName(StringUtils::format("mage_shooter_0001.png"));
...
void BaseMageTower::shoot(float dt)
{
	...
	if(nearestMonster!=NULL && nearestMonster->getCurrHp() > 0)
    {
		...
		towerBase->runAction(Animate::create(AnimationCache::getInstance()->getAnimation(StringUtils::format("level%d_mage_shine",level))));
