...
bool Enemy::init(){
	...
	for(int i = 0; i < 48; i ++)
	{
		...
		temp1 = temp1 + StringUtils::format("%d",i+1);
		temp2 = temp2 + StringUtils::format("%d",i+1);
		temp1 = temp1 + ".png";
		temp2 = temp2 + ".png";
	...
	enemyId = 0;
	labelPage = Label::createWithTTF(StringUtils::format("%d",enemyId + 1), "Marker Felt.ttf", 24);
...
void Enemy::lastPage(Ref* pSender){
	...
	labelPage->setString(StringUtils::format("%d",enemyId + 1));
