...
bool ShopScene::init()
{
	...
	//��ȡ��Ļ�ߴ�
	size = Director::getInstance()->getVisibleSize(); //��ȡ��Ļ��С 
	...
	for(int i=0;i<6;i++)
	{
		count[i]=UserDefault::getInstance()->getIntegerForKey(StringUtils::format(GameManager::getInstance()->SLOTX_SHOP,i).c_str());
	}
	//==========================================================//�������Ʒ
	...
	gemLabel = Label::createWithTTF(StringUtils::format("%d",gem), "SohoGothicProMedium.ttf", 30);
...
void ShopScene::initBackpackIcon()
{
	...
	for(int i=0;i<6;i++){
		auto backPackIcon = Sprite::createWithSpriteFrameName(StringUtils::format("inaps_backPackIcons_000%d.png",i+1));
		backPackIcon->setPosition(156,size.height*(0.72f-a[i]));
		addChild(backPackIcon,3);
		if(count[i] == 0)
			backPackIcon->setVisible(false);
		auto countLabel =  Label::createWithTTF(StringUtils::format("%d",count[i]), "SohoGothicProMedium.ttf", 24); //С�к�������
...
void ShopScene::initPicture()
{
	...
	auto shopIcon1 = ShopIcon::createIcon(1);//ԭ�ӵ�
	shopIcon1->setPosition(Point(640,170));
	addChild(shopIcon1);

	auto shopIcon4 = ShopIcon::createIcon(4);//ҩƿ
	...
	auto shopIcon3 = ShopIcon::createIcon(3);//ը��
...
//---------------------------------------------------------------------------------------------------------------------------//
	 //ȷ��
      ...
      auto menu = Menu::create(confirmButton,buyButton,NULL);  //ʵ���л�  
 ...
 void ShopScene::buySth(int i)
 {
	 if(gem < str_gem[i]){//��ʯ����
		...
		gem = gem - str_gem[i];
		setBackpackIcon(i);
		gemLabel->setString(StringUtils::format("%d",gem));
 ...
 {
	 count[i] ++;
	 countLabels.at(i)->setString(StringUtils::format("%d",count[i]));
	 if(count[i]>0)
 ...
 void ShopScene::setBorad(int type)
 {
	 borad->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(
		 StringUtils::format("inaps_boardDrawings_000%d.png",type)));
	 label3->setString(str_introduction[type-1]);
	 label2->setString(StringUtils::format("%d",str_gem[type-1]));
