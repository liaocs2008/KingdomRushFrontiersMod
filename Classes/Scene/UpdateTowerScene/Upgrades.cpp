...
void Upgrades::initalState()
{
	if(!UserDefault::getInstance()->getBoolForKey("upgrades_icon_sta_isExit",false))
	{
		for(int i=0;i<5;i++)
			for(int j=0;j<6;j++)
			{
				UserDefault::getInstance()->setIntegerForKey(StringUtils::format("upgrades_icon_%d_%d",i,j).c_str(),(int)iconInitSta[i][j]);
			}
	...
	for(int j=0;j<6;j++)
		...
		{			
			...
			if(1==UserDefault::getInstance()->getIntegerForKey(StringUtils::format("upgrades_icon_%d_%d",i,j).c_str()))
			{
...
bool Upgrades::init()
{
	...
	//�������ѡ��������
	auto bg2TileRtSprite = Sprite::createWithSpriteFrameName("upgrades_bg2_tileRight.png");//�ұ߿�
	auto bg2TileLtSprite = Sprite::createWithSpriteFrameName("upgrades_bg2_tileRight.png");//��߿�
	bg2TileLtSprite->setFlippedX(true);//��߿�ת
	...
	//ʣ���������ʾ(��ǩ)
	lastStartLabel = Label::createWithSystemFont(StringUtils::format("%d",startLeft),"Arial",50);
	...
	upNeedStartLabel = Label::createWithSystemFont(StringUtils::format("%d",lastSelectedIcon->priceTag),"Arial",28);
...
bool Upgrades::onIconTouchBegan(Touch* touch, Event* event)
{
	...
	{  	
		...
		upNeedStartLabel->setString(StringUtils::format("%d",lastSelectedIcon->priceTag));
...
void Upgrades::resetStaEnded(Touch* touch , Event* event)
{
	...
	lastStartLabel->setString(StringUtils::format("%d",startLeft));
...
void Upgrades::onBuyButTouchEnded(Touch* touch,Event* event)
{
	...
	UserDefault::getInstance()->setIntegerForKey(
			StringUtils::format("upgrades_icon_%d_%d",lastSelectedIcon->iconRow,lastSelectedIcon->iconColumn).c_str(),
			1);
