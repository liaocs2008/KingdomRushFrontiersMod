...
bool SlotMenu::initWithNum(int num)
{
	...
	}else
	{
		...
		auto label_slot = Label::createWithTTF(StringUtils::format("SLOT %d",num),"Comic_Book.ttf",32,Size::ZERO,TextHAlignment::LEFT,TextVAlignment::TOP);
		...
		auto slot = Label::createWithTTF(StringUtils::format("%d/65",starCountInt),"Comic_Book.ttf",32,Size::ZERO,TextHAlignment::LEFT,TextVAlignment::TOP);
...
{
	UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d",getNum()).c_str(),0);

	for(int i=0;i<13;i++)
	{
		UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_Level_%d",getNum(),i).c_str(),0);
	}

	for(int i=0;i<13;i++)
	{
		UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_Level_%d_star",getNum(),i).c_str(),0);
	}

	UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_DownCount",getNum()).c_str(),0);
 
	UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_NewDown",getNum()).c_str(),0);

	UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_StarLeft",getNum()).c_str(),0);

	UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_Star",getNum()).c_str(),0);

	UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_Gem",getNum()).c_str(),0);

	for(int i = 0;i<6;i++){
		UserDefault::getInstance()->setIntegerForKey(StringUtils::format("Slot%d_Shop_%d",getNum(),i).c_str(),0);
	}
}