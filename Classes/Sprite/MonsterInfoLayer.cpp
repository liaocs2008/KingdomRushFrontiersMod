...
void MonsterInfoLayer::updateMonsterHp(float dt)
{
	if(mBaseMonster->getCurrHp()<=0){
		...
		hpLabel->setString(StringUtils::format("%d/%d",(int)mBaseMonster->getCurrHp(),(int)mBaseMonster->getMaxHp()));
}
...
void MonsterInfoLayer::setMonsterInfo()
{
	...
	hpLabel->setString(StringUtils::format("%d/%d",(int)mBaseMonster->getMaxHp(),(int)mBaseMonster->getMaxHp()));
	forceLabel->setString(StringUtils::format("%d",(int)mBaseMonster->getForce()));
	armorLabel->setString(StringUtils::format("%d",(int)mBaseMonster->getArmor()));
