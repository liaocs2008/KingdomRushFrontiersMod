...
bool ShopIcon::initIcon(int type)
{
	...
	this->type = type;
	icon = Sprite::createWithSpriteFrameName(StringUtils::format("inaps_Icons_000%d.png",type));
	addChild(icon);
	glow = Sprite::createWithSpriteFrameName(StringUtils::format("inaps_IconsGlow_000%d.png",type));
	glow->setScale(2.0f);
	addChild(glow);
	setNotGlowing();
