...
bool BaseBuildIcon::initIcon(int type,int money)
{
	...
	isSelected = false;
	auto test = StringUtils::format("main_icons_00%02d.png",type);
	baseIcon = Sprite::createWithSpriteFrameName(StringUtils::format("main_icons_00%02d.png",type));
	addChild(baseIcon);
	priceIcon = Sprite::createWithSpriteFrameName("price_tag.png");
	priceIcon->setPosition(Point(0,-baseIcon->getContentSize().height/2));
	addChild(priceIcon);
	priceLabel = Label::createWithTTF(StringUtils::format("%d",money),"arial.ttf",20);
...
{
	setMoney(money);
	priceLabel->setString(StringUtils::format("%d",money));
}
...
void BaseBuildIcon::setNotEnoughMoney()
{
	...
	baseIcon->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("main_icons_disabled_00%02d.png",type)));
...
void BaseBuildIcon::setEnoughMoney()
{
	...
	baseIcon->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("main_icons_00%02d.png",type)));
