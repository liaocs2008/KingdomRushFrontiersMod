...
void MapFlag::initStar()
{
	...
	{
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("mapFlag_star_00%02d.png",len));
...
void MapFlag::startDropdownAnimation()
{
	...
	for (int len = 1; len <= 22; len++)
	{
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(
			StringUtils::format("mapFlag_0%03d.png", len));
...
void MapFlag::startSuccessAnimation()
{
	...
	{
		frame = SpriteFrameCache::getInstance()->getSpriteFrameByName(StringUtils::format("mapFlag_0%03d.png",len));
